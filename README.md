...
| `--option viewImageInCurrentTab`            |   Off   | The context menu option "Open [image/video] in new tab" is replaced with the older 'View [image/video]' option, which opens in the current tab unless middle clicked, ctrl+clicked, or shift+clicked. Middle click and ctrl+click open in a new tab, and shift+click opens in a new window. The context menu label may not be updated for most locales (only English for now), but the functionality will still be corrected. |
